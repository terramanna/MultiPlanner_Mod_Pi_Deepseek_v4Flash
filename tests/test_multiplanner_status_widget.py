...
def service_manager_for(module):
    return sys.modules[module.Service.__module__]


def test_pids_to_stop_includes_spawned_children(monkeypatch):
    module = load_widget_module()
    service_manager = service_manager_for(module)
    ...
    monkeypatch.setattr(service_manager, "load_pid_registry", lambda: {"Backend": 28368})
...
def test_pids_to_stop_includes_frontend_wrapper_chain(monkeypatch):
    ...
    monkeypatch.setattr(service_manager, "load_pid_registry", lambda: {"Frontend": 30280})
