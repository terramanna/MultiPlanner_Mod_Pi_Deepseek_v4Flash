...
def test_lvermgeo_sh_provider_lists_dgm1_only() -> None:
    assert provider_dataset_names("lvermgeo-sh") == ("dgm1",)
...
def test_locate_tiles_returns_matching_tile(monkeypatch) -> None:
    monkeypatch.setattr("multiplanner_api.sh._load_index", lambda timeout: [SAMPLE_TILE])
...
def test_locate_tiles_raises_for_oversized_area(monkeypatch) -> None:
    ...
    monkeypatch.setattr("multiplanner_api.sh._load_index", lambda timeout: big_index)
...
def test_load_index_uses_cached_file(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("multiplanner_api.sh._cache_path", lambda: cache_file)
    fetch_called = []
    monkeypatch.setattr(
        "multiplanner_api.sh.requests.get",
        lambda *a, **kw: fetch_called.append(1) or _mock_geojson_response([]),
    )
    result = _load_index(timeout=5)
    assert fetch_called == []
