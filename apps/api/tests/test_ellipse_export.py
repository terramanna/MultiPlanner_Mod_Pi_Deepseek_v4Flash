...
def test_translate_reprojects_the_ellipse_tiff_to_wgs84_utm32(monkeypatch, tmp_path) -> None:
    command = []

    monkeypatch.setattr(
        "multiplanner_api.downloads.subprocess.run",
        lambda args, check: command.extend(args),
    )

    _translate_vrt(Path("gdal_translate.exe"), tmp_path / "dgm1.vrt", tmp_path / "dgm1.tif")
