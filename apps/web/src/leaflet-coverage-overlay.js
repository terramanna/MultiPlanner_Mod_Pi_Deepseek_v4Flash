...
export function createCoverageOverlay(map, state, providerCoverage, coverageToggle, downloadStatus) {
  const updateCoverageOverlay = () => updateCoverageOverlayFor(map, state, providerCoverage, coverageToggle);
  const applyCoverage = (features) => applyProviderCoverage(map, state, providerCoverage, updateCoverageOverlay, features);
  const loadProviderCoverage = () => loadProviderCoverageFor(map, providerCoverage, downloadStatus, applyCoverage);
  map.on("zoomend", updateCoverageOverlay);
  return { loadProviderCoverage, updateCoverageOverlay };
}

function updateCoverageOverlayFor(map, state, providerCoverage, coverageToggle) {
...
function applyProviderCoverage(map, state, providerCoverage, updateCoverageOverlay, featuresByProvider) {
  clearCoverageLayers(map, state);
...
function clearCoverageLayers(map, state) {
...
async function loadProviderCoverageFor(map, providerCoverage, downloadStatus, applyProviderCoverage) {
  const cachedFeatures = loadProviderCoverageCache();
  if (cachedFeatures) applyProviderCoverage(cachedFeatures);
  try {
    const featuresByProvider = await fetchProviderCoverage(providerCoverage);
    ...
    if (!cachedFeatures) downloadStatus.textContent = "Provider coverage outline could not be loaded.";
  }
}

async function fetchProviderCoverage(providerCoverage) {
